...
abstract class OAuthProfileCreator implements ProfileCreator {
    ...
    protected Optional<UserProfile> retrieveUserProfileFromToken(final WebContext context, final Token accessToken) {
        ...
        //Handle EDL's dynamic user profile endpoint by extracting the profile path from the accessToken.
        String profilepath = "";
		try {
			profilepath = extractProfileUrlFromResponse(accessToken.getRawResponse());
		} catch (OAuthException e) {
			e.printStackTrace();
		} catch (IOException e) {
			e.printStackTrace();
		}
        final var profileUrl = profileDefinition.getProfileUrl(accessToken, configuration) + profilepath;
    ...
    private String extractProfileUrlFromResponse(String body) throws IOException, OAuthException {
        ...
        if (matcher.find()) {
            ...
            throw new OAuthException("Cant' find Profile URL from response:" + body, null);
        }
    }
